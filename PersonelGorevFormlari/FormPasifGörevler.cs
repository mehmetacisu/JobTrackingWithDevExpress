...
{
    public partial class FormPasifGörevler : Form
    {
        IsTakipEntities db = new IsTakipEntities();
        public string mail2;
        public FormPasifGörevler()
        {
            InitializeComponent();
        }

        private void FormPasifGörevler_Load(object sender, EventArgs e)
        {
            ...
            var degerler = (from x in db.TblGorevler
                            select new
                            {
                                ...
                                x.GorevAlan,
                                x.Durum
                            }).Where(x => x.GorevAlan == pid && x.Durum == false).ToList();
